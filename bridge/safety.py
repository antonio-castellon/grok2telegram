...
# Things that must never appear in a group say (values from env + shapes).
...
def scrub_outbound(text: str) -> str:
    ...
    # Never echo wake URLs wholesale
    scrubbed = re.sub(
...
def block_reason_for_payload(verb: str, payload: str) -> Kind | None:
    """Return secrets/adult if this command must not reach the GM inbox."""
