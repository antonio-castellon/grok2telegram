...
def _wrap(text: str) -> list[str]:
    raw = text or ""
    # ASCII art / preformatted: keep spaces; hard-chunk instead of dropping tail.
    # Do NOT treat "/" alone as art (every "/cmd …" line would get clipped).
    if any(ch in raw for ch in "+|┌┐└┘│─\\") or "  " in raw:
        if not raw.strip():
            return [_row("")]
        lines: list[str] = []
        while raw:
            lines.append(_row(raw[:INNER]))
            raw = raw[INNER:]
        return lines
    raw = " ".join(raw.split())
    if not raw:
